...
- `scripts/build_semantic_memory.py` – aggregates episodic JSONL into semantic summaries using an LLM-based summarizer by default (no raw-code summaries in the CLI path; callers can still pass a custom summarizer).
- `scripts/build_semantic_from_repo_chunks.py` – convenience to turn repo_chunks exports into per-repo semantic summaries (no sampling; uses all chunks) using an LLM summarizer only (no raw code stored as `summary_text`).
